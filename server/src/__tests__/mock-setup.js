...
module.exports.mockSetup = async ({ eventDS }) => {
	...
	await eventDS.updateSnapshot();
	await eventDS.add({ event: evn.GAME_JOINED, payload: { data: [ptokens[1], gtokens[0]] }});
	await eventDS.updateSnapshot();
	await eventDS.add({ event: evn.GAME_OPENED, payload: { name: "Fred's Game", data: [ ptokens[4] ]}}).then(r => gtokens.push(r.event.token));
	await eventDS.updateSnapshot();
